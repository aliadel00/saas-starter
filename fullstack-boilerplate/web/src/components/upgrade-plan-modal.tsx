...
export function UpgradePlanModal({ open, onClose }: UpgradePlanModalProps) {
  const { tenant, refreshUsage } = useAuthContext();
  const [selected, setSelected] = useState<SubscriptionPlan | null>(null);

  const [changePlan, { loading }] = useMutation(CHANGE_PLAN_MUTATION, {
    refetchQueries: [
      { query: MY_TENANT_QUERY },
      { query: TENANT_USAGE_QUERY },
    ],
    onCompleted: () => {
  ...
  const handleUpgrade = useCallback(() => {
    if (!selected) return;
    changePlan({ variables: { input: { plan: selected } } });
  }, [selected, changePlan]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="mx-4 w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {PLANS.map(({ plan, label, price, features }) => {
